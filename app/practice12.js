let friendFaces = [{
    "id": 1,
    "name": {
      "first": "Eb",
      "last": "Keri"
    },
    "jobTitle": "Environmental Tech",
    "joined": 2002,
    "friends": 29,
    "avatar": "https://robohash.org/quiaquicommodi.bmp?size=200x200&set=set1",
    "city": "Fort Lauderdale",
    "state": "Florida"
  }, {
    "id": 2,
    "name": {
      "first": "Emmalynn",
      "last": "Edmands"
    },
    "jobTitle": "Product Engineer",
    "joined": 2012,
    "friends": 32,
    "avatar": "https://robohash.org/nequeconsequaturnesciunt.jpg?size=200x200&set=set1",
    "city": "Pensacola",
    "state": "Florida"
  }, {
    "id": 3,
    "name": {
      "first": "Buck",
      "last": "Smardon"
    },
    "jobTitle": "Speech Pathologist",
    "joined": 2016,
    "friends": 73,
    "avatar": "https://robohash.org/nemoquosporro.bmp?size=200x200&set=set1",
    "city": "Minneapolis",
    "state": "Minnesota"
  }, {
    "id": 4,
    "name": {
      "first": "Debbi",
      "last": "Aristide"
    },
    "jobTitle": "GIS Technical Architect",
    "joined": 2001,
    "friends": 48,
    "avatar": "https://robohash.org/liberohicex.png?size=200x200&set=set1",
    "city": "Terre Haute",
    "state": "Indiana"
  }, {
    "id": 5,
    "name": {
      "first": "Leela",
      "last": "Osgerby"
    },
    "jobTitle": "Marketing Manager",
    "joined": 2014,
    "friends": 41,
    "avatar": "https://robohash.org/quossaepeerror.jpg?size=200x200&set=set1",
    "city": "Fresno",
    "state": "California"
  }, {
    "id": 6,
    "name": {
      "first": "Sid",
      "last": "Bondesen"
    },
    "jobTitle": "Clinical Specialist",
    "joined": 2006,
    "friends": 97,
    "avatar": "https://robohash.org/dolorestemporasoluta.jpg?size=200x200&set=set1",
    "city": "Bronx",
    "state": "New York"
  }, {
    "id": 7,
    "name": {
      "first": "Rina",
      "last": "Potte"
    },
    "jobTitle": "Geological Engineer",
    "joined": 2014,
    "friends": 73,
    "avatar": "https://robohash.org/eaquideserunt.bmp?size=200x200&set=set1",
    "city": "Philadelphia",
    "state": "Pennsylvania"
  }, {
    "id": 8,
    "name": {
      "first": "Terry",
      "last": "Pottie"
    },
    "jobTitle": "Nurse Practicioner",
    "joined": 2009,
    "friends": 92,
    "avatar": "https://robohash.org/sitvoluptatibuscupiditate.bmp?size=200x200&set=set1",
    "city": "Boise",
    "state": "Idaho"
  }, {
    "id": 9,
    "name": {
      "first": "Mathilde",
      "last": "Christmas"
    },
    "jobTitle": "Information Systems Manager",
    "joined": 2007,
    "friends": 15,
    "avatar": "https://robohash.org/pariatursedsed.jpg?size=200x200&set=set1",
    "city": "Raleigh",
    "state": "North Carolina"
  }, {
    "id": 10,
    "name": {
      "first": "Abran",
      "last": "O'Corrin"
    },
    "jobTitle": "Biostatistician I",
    "joined": 2010,
    "friends": 71,
    "avatar": "https://robohash.org/dolorinciduntaperiam.jpg?size=200x200&set=set1",
    "city": "Topeka",
    "state": "Kansas"
  }, {
    "id": 11,
    "name": {
      "first": "Creight",
      "last": "Tyrone"
    },
    "jobTitle": "Technical Writer",
    "joined": 2011,
    "friends": 59,
    "avatar": "https://robohash.org/etinvoluptatem.bmp?size=200x200&set=set1",
    "city": "El Paso",
    "state": "Texas"
  }, {
    "id": 12,
    "name": {
      "first": "Allyson",
      "last": "Ofener"
    },
    "jobTitle": "Community Outreach Specialist",
    "joined": 2017,
    "friends": 18,
    "avatar": "https://robohash.org/enimnamrerum.jpg?size=200x200&set=set1",
    "city": "Memphis",
    "state": "Tennessee"
  }, {
    "id": 13,
    "name": {
      "first": "Alec",
      "last": "Ferronier"
    },
    "jobTitle": "Assistant Manager",
    "joined": 2010,
    "friends": 7,
    "avatar": "https://robohash.org/velutcorporis.jpg?size=200x200&set=set1",
    "city": "Colorado Springs",
    "state": "Colorado"
  }, {
    "id": 14,
    "name": {
      "first": "Whitaker",
      "last": "Humbatch"
    },
    "jobTitle": "Financial Analyst",
    "joined": 2016,
    "friends": 67,
    "avatar": "https://robohash.org/doloremetducimus.jpg?size=200x200&set=set1",
    "city": "Houston",
    "state": "Texas"
  }, {
    "id": 15,
    "name": {
      "first": "Jillian",
      "last": "Yankov"
    },
    "jobTitle": "Operator",
    "joined": 2012,
    "friends": 89,
    "avatar": "https://robohash.org/inquiavoluptatem.jpg?size=200x200&set=set1",
    "city": "Canton",
    "state": "Ohio"
  }, {
    "id": 16,
    "name": {
      "first": "Gunner",
      "last": "Bolsover"
    },
    "jobTitle": "Sales Associate",
    "joined": 2012,
    "friends": 51,
    "avatar": "https://robohash.org/autemetconsequatur.png?size=200x200&set=set1",
    "city": "San Jose",
    "state": "California"
  }, {
    "id": 17,
    "name": {
      "first": "Ferd",
      "last": "McKitterick"
    },
    "jobTitle": "Environmental Specialist",
    "joined": 2014,
    "friends": 96,
    "avatar": "https://robohash.org/dictautpariatur.png?size=200x200&set=set1",
    "city": "Atlanta",
    "state": "Georgia"
  }, {
    "id": 18,
    "name": {
      "first": "Neale",
      "last": "Miguet"
    },
    "jobTitle": "VP Sales",
    "joined": 2012,
    "friends": 42,
    "avatar": "https://robohash.org/providentminimaexplicabo.png?size=200x200&set=set1",
    "city": "Providence",
    "state": "Rhode Island"
  }, {
    "id": 19,
    "name": {
      "first": "Giralda",
      "last": "Ivushkin"
    },
    "jobTitle": "Electrical Engineer",
    "joined": 2014,
    "friends": 25,
    "avatar": "https://robohash.org/sedlaboreeaque.bmp?size=200x200&set=set1",
    "city": "West Palm Beach",
    "state": "Florida"
  }, {
    "id": 20,
    "name": {
      "first": "Pepi",
      "last": "Gradly"
    },
    "jobTitle": "Database Administrator IV",
    "joined": 2014,
    "friends": 84,
    "avatar": "https://robohash.org/etsuntdolores.png?size=200x200&set=set1",
    "city": "Salinas",
    "state": "California"
  }]
  //using filter
  //write out a function that prints out the avatar urls
  //of those whose url is a png

//   let pngs = friendFaces.filter(friend => friend.avatar.includes('png'));
//   pngs.forEach(png => console.log(png.avatar));

 //we want to anonomize the sex for a study so lets
 // use map to create an array that contains the id, jobTitle 
 // and city of all of our friends
 // the print out that array.

// let anon = friendFaces.map(friend =>  {
//     return {id: friend.id, jobTitle: friend.jobTitle, city: friend.city}
// });
// console.log(anon);

 //of those who joined before 2010 what is the average
 //friends per person
 //use filter then use reduce 

//  let early = friendFaces.filter(friendFace => friendFace.joined < 2010);
//  let average = early.reduce((first, second) => first + second.friends, 0) / early.length;
//  console.log(average);

 //how many friends on average do people who have
 //engineer or manager in their title have
 //use filter then reduce

 //chain together map and reduce to first get
 //an array of just friend counts and then to
 //reduce that array to get a total of all of
 //the friends



  